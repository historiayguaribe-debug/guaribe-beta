...
def init_db_pool():
    ...
    try:
        ...
        logger.info("✅ Pool de conexiones creado (min=1, max=20)")
...
def init_db():
    ...
    try:
        ...
        conn.commit()
        logger.info("✅ Base de datos lista con índices optimizados")
    except Exception as e:
...
def handle_buttons(m):
    ...
    if accion == "email":
        # Extraer destino, asunto y cuerpo del texto
        try:
            # Buscar patrón: "enviar correo a correo@ejemplo.com asunto ..."
    ...
    try:
        ...
        if es_pregunta_simple(texto):
            ...
            if GROQ_API_KEY:
                try:
                    resp = orquestador._consultar_groq(mensajes)
                except:
                    resp = orquestador.consultar(mensajes, usar_busqueda=False)
            else:
                resp = orquestador.consultar(mensajes, usar_busqueda=False)
...
@app.route('/')
def home():
    return jsonify({"status": "ok", "bot": "Guaribe 8.0 Optimizado - Lenguaje Natural"}), 200
...
if __name__ == "__main__":
    logger.info("🚀 Iniciando Guaribe 8.0 Optimizado (Lenguaje Natural)...")
    init_db_pool()
    init_db()
    ...
    logger.info("🧠 Cerebros disponibles: DeepSeek, Groq")
    logger.info("⚡ Optimizaciones: cache, pool, índices, compresión condicional, lenguaje natural")
